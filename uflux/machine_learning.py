...
class HybridGPRForestRegressor(BaseEstimator, RegressorMixin):
    """
    Hybrid model combining a base regressor (RandomForest or XGBoost)
    with a Gaussian Process on residuals to produce probabilistic predictions.

    Parameters
    ----------
    base_model : str, default='rf'
        Base learner type: 'rf' for Random Forest, 'xgb' for XGBoost.
    base_params : dict, optional
        Parameters for the base model.
    gpr_kernel : sklearn.gaussian_process.kernels.Kernel, optional
        Kernel for the Gaussian Process. Default = RBF + WhiteKernel.
    gpr_alpha : float, default=1e-2
        Regularization parameter for GPR.
    random_state : int, optional
        Random seed.
    """

    def __init__(self, base_model='rf', base_params=None,
                 gpr_kernel=None, gpr_alpha=1e-2, random_state=None):

        self.base_model = base_model
        self.base_params = base_params or {}
        self.gpr_kernel = gpr_kernel or (RBF(length_scale=1.0) + WhiteKernel(noise_level=1e-2))
    ...
    def _init_base_model(self):
        ...
        if self.base_model == 'rf':
            ...
            raise ValueError("base_model must be 'rf' or 'xgb'")
        return model
    ...
    def fit(self, X, y):
        ...
        self._base = self._init_base_model()
        self._base.fit(X, y)

        # Step 2: Compute residuals
        residuals = y - self._base.predict(X)

        # Step 3: Fit Gaussian Process on residuals
        self._gpr = GaussianProcessRegressor(
            kernel=clone(self.gpr_kernel),
    ...
    def predict(self, X, return_std=False):
        ...
        # Base predictions
        y_base = self._base.predict(X)

        # Residual correction and uncertainty
        y_residual_mean, y_residual_std = self._gpr.predict(X, return_std=True)

        y_mean = y_base + y_residual_mean

        if return_std:
            # Try to estimate base uncertainty (for Random Forest only)
            if isinstance(self._base, RandomForestRegressor):
                all_preds = np.stack([t.predict(X) for t in self._base.estimators_])
                rf_var = np.var(all_preds, axis=0)
                total_std = np.sqrt(rf_var + y_residual_std ** 2)
            else:
                total_std = y_residual_std
...
# 1️⃣ Hybrid using Random Forest
model_rf = HybridGPRForestRegressor(base_model='rf', random_state=42)
model_rf.fit(X_train, y_train)
y_mean_rf, y_std_rf = model_rf.predict(X_test, return_std=True)

# 2️⃣ Hybrid using XGBoost
model_xgb = HybridGPRForestRegressor(base_model='xgb', random_state=42)
model_xgb.fit(X_train, y_train)
y_mean_xgb, y_std_xgb = model_xgb.predict(X_test, return_std=True)

print(f"RF Hybrid Prediction: {y_mean_rf[:3]} ± {y_std_rf[:3]}")
print(f"XGB Hybrid Prediction: {y_mean_xgb[:3]} ± {y_std_xgb[:3]}")
